// ==================== 生成语义 embedding 作者向量库（智谱 embedding-3）====================
// 用法（凭证在 .env 或环境变量）：ZHIPU_API_KEY
// 运行：node scripts/build-embedding-vectors.mjs
// 可选 ENV：EMB_MODEL（默认 embedding-3，与 api/xfyunEmbed.js 一致）、
//           AUTHOR_CAP（每位作者样本数上限，默认 6）、EMBED_DELAY_MS
// 策略：每位作者的每条样本单独 embed（保留叙事/对话/哲思等不同风格面），
// 再逐维取均值得到作者中心向量。比"整批拼接后 embed"更能覆盖作者的多面文风。
// 输出：api/data/authorEmbeddings.js（author name → 2048 维向量）
...
const AUTHOR_CAP = Number(process.env.AUTHOR_CAP || 6)
...
if (!embedReady()) {
  console.error('缺少 ZHIPU_API_KEY（可从 .env 或环境变量读）。')
  process.exit(1)
}
...
for (let idx = 0; idx < names.length; idx++) {
  ...
  if (!vecs.length) {
    console.error(`  ${name} 全部样本失败，跳过（该作者将无向量）`)
    continue
  }
  const mean = new Array(dim).fill(0)
  for (const v of vecs) for (let i = 0; i < dim; i++) mean[i] += v[i] / vecs.length
  totalChars += samples.join('').length
  map[name] = mean.map((v) => Math.round(v * 1e4) / 1e4)
  console.log(`[${idx + 1}/${names.length}] ${name}  dim=${dim}  samples=${vecs.length}  chars=${samples.join('').length}`)
}
